...
class ControlROS(Node):
        ...
        while rclpy.ok() and not self.controller.shutdown_event.is_set():
            if self.controller.path is None:
                ...
                with self.controller.path_condition:
                    while self.controller.path is None and not self.controller.shutdown_event.is_set():
    ...
    def setup_pub_sub(self):
        ...
        self._follow_path_srv = self.create_service(FollowPath, "~/follow_path", self.cb_path)
        ...
        self._path_pub = self.create_publisher(PoseArray, "~/path/poses", 1)

        self._real_poses = []
        self._real_path_pub = self.create_publisher(PoseArray, "~/real_path/poses", 1)
    ...
    def follow_path_with_speed(self, path_xyt, speed):
        ...
        path_xytv = time_parameterize_ramp_up_ramp_down(path_xyt, speed, self.controller.min_speed)
    ...
    def cb_path(self, request, response):
        ...
        path_xyt = np.array([utils.pose_to_particle(pose.pose) for pose in transformed_path])
...
def get_ros_params(node):
    ...
    if controller_type == "pid":
        ...
        permissible_region, map_info = utils.get_map(node, "/static_map")
        params["permissible_region"] = permissible_region
        params["map_info"] = map_info

    else:
        raise RuntimeError(f"'{controller_type}' is not a controller. You must specify 'pid', 'pp', or 'mpc'")
