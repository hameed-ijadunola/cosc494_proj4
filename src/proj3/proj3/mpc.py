...
class ModelPredictiveController(BaseController):
    def __init__(
        ...
        **kwargs,
        ...
        self.motion_model = KinematicCarMotionModel(self.car_length, **self.kinematics_params)
    ...
    def check_collisions_in_map(self, poses):
        ...
        rot = np.array([[np.cos(thetas), -np.sin(thetas)], [np.sin(thetas), np.cos(thetas)]]).T

        self.bbox_map = np.matmul(self.car_bbox[np.newaxis, ...], rot) + points[:, np.newaxis]

        bbox_idx = self.bbox_map.astype(int)

        np.clip(bbox_idx[..., 1], 0, self.obstacle_map.shape[0] - 1, out=bbox_idx[..., 1])
        np.clip(bbox_idx[..., 0], 0, self.obstacle_map.shape[1] - 1, out=bbox_idx[..., 0])

        self.collisions = self.obstacle_map[bbox_idx[..., 1], bbox_idx[..., 0]].max(axis=1)
