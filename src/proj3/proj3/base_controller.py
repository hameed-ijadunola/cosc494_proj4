...
class BaseController(Node):
  ...
  def get_reference_index(self, pose, path_xytv, distance_lookahead):
    ...
    with self.path_condition:
      ...
      # BEGIN QUESTION 1.1
      path_xy = path_xytv[:, :2]
      pose_xy = np.array(pose[:2])

      # Compute distances
      distances = np.linalg.norm(path_xy - pose_xy, axis=1)

      # Closest point
      closest_index = np.argmin(distances)

      # Forward points only
      forward_distances = distances[closest_index:]

      # Find first point beyond lookahead
      candidates = np.where(forward_distances >= distance_lookahead)[0]

      if len(candidates) > 0:
        lookahead_index = closest_index + candidates[0]

        # Optional refinement
        if lookahead_index > closest_index:
          prev_index = lookahead_index - 1
          if abs(distances[prev_index] - distance_lookahead) <= abs(
              distances[lookahead_index] - distance_lookahead
          ):
            return prev_index
