...
from proj4 import problems
from proj4 import samplers


class Roadmap(object):
    def __init__(self, problem, sampler, num_vertices, connection_radius, lazy=False, saveto=None):
    ...
    def construct(self):
        ...
        if not self.lazy:
            self.weighted_edges = self.check_weighted_edges_validity(self.weighted_edges)
        ...
        vbunch = [(i, dict(config=config)) for i, config in zip(np.arange(self.num_vertices, dtype=int), self.vertices)]
