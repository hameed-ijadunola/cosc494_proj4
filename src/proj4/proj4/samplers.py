...
class HaltonSampler(Sampler):
    ...
    def sample(self, num_samples):
        ...
        return lower + batch * (upper - lower)
        # END QUESTION 1.1
...
class LatticeSampler(Sampler):
    ...
    def sample(self, num_samples):
        ...
        steps = [np.arange(self.extents[i, 0] + resolution / 2, self.extents[i, 1], resolution) for i in range(self.dim)]
        meshed = np.array(np.meshgrid(*steps)).reshape((self.dim, -1)).T
