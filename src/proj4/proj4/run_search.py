...
from proj4 import rrt
from proj4.problems import R2Problem, SE2Problem
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="test (lazy) A* or RRT or RRT* on R2 and SE2 problems")
    ...
    r2_parser.add_argument("-s", "--start", nargs=2, type=float, required=True)
    r2_parser.add_argument("-g", "--goal", nargs=2, type=float, required=True)

    se2_parser = subparsers.add_parser("se2", help="SE2 options")
    se2_parser.add_argument("-s", "--start", nargs=3, type=float, required=True)
    se2_parser.add_argument("-g", "--goal", nargs=3, type=float, required=True)
